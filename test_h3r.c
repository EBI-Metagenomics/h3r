...
static void test_unpack(void)
{
  FILE *fp = fopen("h3r.mp", "rb");
  ok(fp);

  struct h3r *result = h3r_new();
  ok(fp);

  eq(h3r_unpack(result, fp), 0);
  fclose(fp);
...
static void test_targets_table(void)
{
  ...
  ok(fp);
  h3r_print_targets_table(result, fp);
  ok(!fclose(fp));
...
static void test_domains_table(void)
{
  ...
  ok(fp);
  h3r_print_domains_table(result, fp);
  ok(!fclose(fp));
