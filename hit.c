...
int h3r_hit_init(struct hit *hit)
{
...
static void shrink(struct hit *hit, unsigned ndomains)
{
  for (unsigned i = ndomains; i < hit->ndomains; ++i)
    h3r_domain_cleanup(hit->domains + i);

  hit->ndomains = ndomains;
}

int h3r_hit_setup(struct hit *hit, unsigned ndomains)
{
...
void h3r_hit_cleanup(struct hit *hit)
{
  DEL(hit->name);
  DEL(hit->acc);
  DEL(hit->desc);
...
int h3r_hit_pack(struct hit const *hit, struct lip_file *f)
{
...
int h3r_hit_unpack(struct hit *hit, struct lip_file *f)
{
  int rc = H3R_EUNPACK;

  if (!h3r_expect_array_size(f, 20)) goto cleanup;

  if ((rc = h3r_read_string(f, &hit->name))) goto cleanup;
  if ((rc = h3r_read_string(f, &hit->acc))) goto cleanup;
  if ((rc = h3r_read_string(f, &hit->desc))) goto cleanup;
  ...
  if (!h3r_expect_key(f, "domains")) goto cleanup;

  unsigned size = 0;
  if (!lip_read_array_size(f, &size)) goto cleanup;
  if ((rc = h3r_hit_setup(hit, size))) goto cleanup;

  for (unsigned i = 0; i < hit->ndomains; ++i)
  {
    if ((rc = h3r_domain_unpack(hit->domains + i, f))) goto cleanup;
  }
