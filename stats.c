...
void h3r_stats_init(struct stats *stats) { memset(stats, 0, sizeof(*stats)); }

int h3r_stats_pack(struct stats const *stats, struct lip_file *f)
{
...
int h3r_stats_unpack(struct stats *stats, struct lip_file *f)
{
  if (!h3r_expect_array_size(f, 13)) return H3R_EUNPACK;
