...
int h3r_alidisplay_init(struct alidisplay *ad)
{
...
void h3r_alidisplay_cleanup(struct alidisplay *ad)
{
  DEL(ad->rfline);
  DEL(ad->mmline);
  DEL(ad->csline);
...
{
  if (presence) lip_write_cstr(f, str);
  else lip_write_cstr(f, "");
}

int h3r_alidisplay_pack(struct alidisplay const *ad, struct lip_file *f)
{
...
int h3r_alidisplay_unpack(struct alidisplay *ad, struct lip_file *f)
{
  int rc = H3R_EUNPACK;

  if (!h3r_expect_array_size(f, 22)) goto cleanup;

  if (!lip_read_int(f, &ad->presence)) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->rfline))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->mmline))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->csline))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->model))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->mline))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->aseq))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->ntseq))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->ppline))) goto cleanup;
  lip_read_int(f, &ad->N);

  if ((rc = h3r_read_string(f, &ad->hmmname))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->hmmacc))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->hmmdesc))) goto cleanup;
  lip_read_int(f, &ad->hmmfrom);
  lip_read_int(f, &ad->hmmto);
  lip_read_int(f, &ad->M);

  if ((rc = h3r_read_string(f, &ad->sqname))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->sqacc))) goto cleanup;
  if ((rc = h3r_read_string(f, &ad->sqdesc))) goto cleanup;
...
void h3r_alidisplay_print(struct alidisplay const *ad, FILE *f)
{
  ...
  unsigned namewidth = h3r_max(strlen(hmmname), strlen(seqname));
  unsigned coordwidth =
      h3r_max(h3r_max(textwidth(ad->hmmfrom), textwidth(ad->hmmto)),
              h3r_max(textwidth(ad->sqfrom), textwidth(ad->sqto)));

  unsigned aliwidth = h3r_zero_clip(120 - namewidth - 2 * coordwidth - 5);
  ...
  for (unsigned pos = 0; pos < ad->N; pos += aliwidth)
  {
    ...
    unsigned k2 = k1 + nk - 1;
    unsigned i2 = 0;
    if (ad->sqfrom < ad->sqto) i2 = (unsigned)(i1 + ni - 1);
    else i2 = (unsigned)(i1 - ni + 1); // revcomp hit for DNA

    if (ad->presence & CSLINE_PRESENT)
    {
      strncpy(buf, ad->csline + pos, aliwidth);
      h3r_echo(f, "  %*s %s CS", namewidth + coordwidth + 1, "", buf);
    }
    if (ad->presence & RFLINE_PRESENT)
    {
      strncpy(buf, ad->rfline + pos, aliwidth);
      h3r_echo(f, "  %*s %s RF", namewidth + coordwidth + 1, "", buf);
    }
    if (ad->presence & MMLINE_PRESENT)
    {
      strncpy(buf, ad->mmline + pos, aliwidth);
      h3r_echo(f, "  %*s %s MM", namewidth + coordwidth + 1, "", buf);
    }

    strncpy(buf, ad->model + pos, aliwidth);
    h3r_echo(f, "  %*s %*d %s %-*d", namewidth, hmmname, coordwidth, k1, buf,
             coordwidth, k2);

    strncpy(buf, ad->mline + pos, aliwidth);
    h3r_echo(f, "  %*s %s", namewidth + coordwidth + 1, " ", buf);

    strncpy(buf, ad->aseq + pos, aliwidth);
    if (ni > 0)
    {
      h3r_echo(f, "  %*s %*u %s %-*u", namewidth, seqname, coordwidth, i1, buf,
               coordwidth, i2);
    }
    else
    {
      h3r_echo(f, "  %*s %*s %s %*s", namewidth, seqname, coordwidth, "-", buf,
               coordwidth, "-");
    }
    ...
    if (ad->ppline != 0)
    {
      ...
      h3r_echo(f, "  %*s %s PP", namewidth + coordwidth + 1, "", buf);
    }
