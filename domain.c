...
void h3r_domain_init(struct domain *dom)
{
  memset(dom, 0, offsetof(struct domain, ad));
  h3r_alidisplay_init(&dom->ad);
}

static int grow_scores(struct domain *dom, unsigned size)
{
  size_t sz = size * sizeof(*dom->pos_score);
  if (!(dom->pos_score = h3r_reallocf(dom->pos_score, sz)))
  {
    h3r_domain_cleanup(dom);
    return H3R_ENOMEM;
  }
  return 0;
}
...
int h3r_domain_setup(struct domain *dom, unsigned scores_size)
{
...
void h3r_domain_cleanup(struct domain *dom)
{
  free(dom->pos_score);
  dom->pos_score = NULL;
  dom->pos_score_size = 0;
  h3r_alidisplay_cleanup(&dom->ad);
}

int h3r_domain_pack(struct domain const *dom, struct lip_file *f)
{
...
int h3r_domain_unpack(struct domain *dom, struct lip_file *f)
{
  int rc = H3R_EUNPACK;

  if (!h3r_expect_array_size(f, 14)) goto cleanup;
  ...
  if ((rc = h3r_domain_setup(dom, size))) goto cleanup;

  for (unsigned long i = 0; i < dom->pos_score_size; i++)
    lip_read_float(f, dom->pos_score + i);

  if (!h3r_expect_map_size(f, 1)) goto cleanup;
  if (!h3r_expect_key(f, "alidisplay")) goto cleanup;

  if ((rc = h3r_alidisplay_unpack(&dom->ad, f))) goto cleanup;
