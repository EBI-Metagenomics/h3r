...
void h3r_tophits_init(struct tophits *th) { memset(th, 0, sizeof(*th)); }
...
static int grow(struct tophits *th, unsigned nhits)
{
  ...
  {
    if ((rc = h3r_hit_init(th->hits + i))) goto cleanup;
    ++th->nhits;
  }
...
static void shrink(struct tophits *th, unsigned nhits)
{
  for (unsigned i = nhits; i < th->nhits; ++i)
    h3r_hit_cleanup(th->hits + i);

  th->nhits = nhits;
}

int h3r_tophits_setup(struct tophits *th, unsigned nhits)
{
...
void h3r_tophits_cleanup(struct tophits *th)
{
...
int h3r_tophits_pack(struct tophits const *th, struct lip_file *f)
{
  lip_write_array_size(f, 5);

  lip_write_map_size(f, 1);
  lip_write_cstr(f, "hits");
  if (!lip_write_array_size(f, th->nhits)) return H3R_EPACK;
  ...
  return lip_file_error(f) ? H3R_EPACK : 0;
}

int h3r_tophits_unpack(struct tophits *th, struct lip_file *f)
{
  int rc = H3R_EUNPACK;

  if (!h3r_expect_array_size(f, 5)) goto cleanup;

  if (!h3r_expect_map_size(f, 1)) goto cleanup;
  if (!h3r_expect_key(f, "hits")) goto cleanup;

  unsigned size = 0;
  if (!lip_read_array_size(f, &size)) goto cleanup;

  if ((rc = h3r_tophits_setup(th, size))) goto cleanup;

  for (unsigned i = 0; i < th->nhits; ++i)
  {
    if ((rc = h3r_hit_unpack(th->hits + i, f))) goto cleanup;
  }
...
static unsigned max_shown_length(struct tophits const *h)
{
  ...
  {
    max = h3r_max(max, (unsigned)strlen(h->hits[i].acc));
    max = h3r_max(max, (unsigned)strlen(h->hits[i].name));
  }
  return max;
}
...
{
  char symbol = ' ';
  if (hit->flags & p7_IS_NEW) symbol = '+';
  else if (hit->flags & p7_IS_DROPPED) symbol = '-';
  return symbol;
}
...
void h3r_tophits_print_targets(struct tophits const *th, FILE *f, double Z)
{
  unsigned namew = h3r_max(8U, max_shown_length(th));
  unsigned descw = h3r_max(32U, h3r_zero_clip(120 - namew - 61));

  h3r_echo(f, "Scores for complete sequence (score includes all domains):");

  h3r_echo(f, "  %22s  %22s  %8s", " --- full sequence ---",
           " --- best 1 domain ---", "-#dom-");

  h3r_echo(f, "  %9s %6s %5s  %9s %6s %5s  %5s %2s  %-*s %s", "E-value",
           " score", " bias", "E-value", " score", " bias", "  exp", "N", namew,
           "Model", "Description");

  h3r_echo(f, "  %9s %6s %5s  %9s %6s %5s  %5s %2s  %-*s %s", "-------",
           "------", "-----", "-------", "------", "-----", " ----", "--",
           namew, "--------", "-----------");
  ...
  for (unsigned i = 0; i < th->nhits; i++)
  {
    ...
    {
      h3r_echo(f, "  ------ inclusion threshold ------");
...
void h3r_tophits_print_domains(struct tophits const *th, FILE *f, double Z,
                               double domZ)
{
  h3r_echo(f, "Domain annotation for each model (and alignments):");
  ...
  for (unsigned i = 0; i < th->nhits; i++)
  {
    ...
    unsigned descw = h3r_max(32U, h3r_zero_clip(120 - namew - 5));

    h3r_echo(f, ">> %s  %-.*s", name, descw, hit->desc);

    if (hit->nreported == 0)
    {
      h3r_echo(f, "   [No individual domains that satisfy reporting "
    ...
    for (unsigned j = 0; j < hit->ndomains; j++)
    {
      ...
      h3r_echo(f, " %4.2f", prob_ali_res(dom));
    }
    ...
    for (unsigned j = 0; j < hit->ndomains; j++)
    {
      ...
      h3r_echo(f, ";  conditional E-value: %.2g", evalue(dom->lnP, domZ));
...
static unsigned max_acc_length(struct tophits const *th)
{
  unsigned max = 0;
  for (unsigned i = 0; i < th->nhits; i++)
    max = h3r_max(max, (unsigned)strlen(th->hits[i].acc));
  return max;
}
...
static unsigned qname_width(struct tophits const *th)
{
  ...
  for (unsigned i = 0; i < th->nhits; i++)
  {
    ...
    {
      struct domain const *dom = hit->domains + j;
      width = h3r_max(width, (unsigned)strlen(dom->ad.sqname));
    }
  }
  return width;
}
...
{
  h3r_echo(f, "#%*s %22s %22s %33s", w.tname + w.qname + w.tacc + w.qacc + 2,
...
{
  struct header_width w = {
      qname_width(th), h3r_max(10U, (unsigned)strlen(qacc)),
      h3r_max(20U, max_name_length(th)), h3r_max(10U, max_acc_length(th))};
...
{
  h3r_echo(f, "#%*s %22s %40s %11s %11s %11s",
                                     ...
                                     double domZ)
{
  ...
  for (unsigned i = 0; i < th->nhits; i++)
  {
    struct hit *const hit = th->hits + i;
    struct domain const *dom = hit->domains + hit->best_domain;
    w.qname = h3r_max(w.qname, (unsigned)strlen(dom->ad.sqname));
  }

  w.tname = h3r_max(w.tname, max_name_length(th));
  w.qacc = h3r_max(w.qacc, (unsigned)strlen(qacc));
  w.tacc = h3r_max(w.tacc, max_acc_length(th));
